            ...
            <section className="mx-auto flex w-full max-w-3xl flex-col gap-6 pb-16 pt-10">
                <Card className="border border-border/40 bg-card">
                    ...
                    <CardContent className="space-y-4 text-sm text-muted-foreground">
                        <p>
                            요청하신 기업 정보를 불러오지 못했어요. URL을 다시 확인하시거나
                            목록으로 돌아가 주세요.
                        </p>
            ...
            <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 sm:px-6">
                <div className="rounded-[32px] border border-slate-200 bg-white px-6 py-7 shadow-[0_25px_80px_rgba(148,163,184,0.25)] dark:border-white/10 dark:bg-slate-950 dark:shadow-[0_35px_120px_rgba(15,23,42,0.65)]">
                    <div className="flex flex-wrap items-center justify-between gap-3 text-sm">
                            <Button
                                variant="ghost"
                                className="px-0 text-muted-foreground transition hover:text-primary"
                                onClick={handleBack}
                            >
                                ← 목록으로 돌아가기
                            </Button>
                    ...
                    <div className="mt-6 grid gap-8 lg:grid-cols-[minmax(0,1.6fr)_minmax(0,0.9fr)]">
                        <div className="space-y-6">
                            <div className="space-y-3">
                                <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">{company.industry}</p>
                                <h1 className="text-4xl font-semibold text-foreground">{company.name}</h1>
                                <p className="text-base leading-relaxed text-muted-foreground">{company.summary}</p>
                            </div>
                        ...
                        <div className="rounded-[28px] border border-slate-200/70 bg-gradient-to-br from-primary/5 via-purple-200/40 to-white p-6 text-center shadow-inner shadow-primary/20 dark:border-white/10 dark:from-primary/10 dark:via-purple-500/10 dark:to-transparent">
                            <p className="text-xs uppercase tracking-[0.2em] text-primary/70">Donut Grade</p>
                            <p className="mt-3 text-6xl font-semibold text-primary-foreground">
                                {donutGrade.toFixed(1)}
                            </p>
                            <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
                                성장력·수익성·안정성·모멘텀·밸류/리스크를 0~100점으로 정규화한 Donut Report 전용
                                종합 점수입니다.
                            </p>
                            <Separator className="my-5 bg-slate-200/60 dark:bg-white/10" />
                            <div className="grid gap-3 text-left text-xs text-muted-foreground">
                                <div className="rounded-2xl border border-slate-200/70 bg-white/80 px-4 py-3 dark:border-white/10 dark:bg-white/5">
                                    ...
                                    <p className="mt-1 text-sm font-semibold text-foreground">{company.brandLabel}</p>
                                </div>
                                <div className="rounded-2xl border border-slate-200/70 bg-white/80 px-4 py-3 dark:border-white/10 dark:bg-white/5">
                                    <p className="text-[0.6rem] uppercase tracking-[0.25em] text-slate-500 dark:text-white/50">
                                        매출 규모
                                    </p>
                                    <p className="mt-1 text-sm font-semibold text-foreground">{company.revenueScale}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="grid gap-6 lg:grid-cols-[minmax(0,1.7fr)_minmax(0,0.85fr)]">
                    <div className="space-y-6">
                        <Card className="border border-border/30 bg-card/60 backdrop-blur">
                            <CardHeader className="space-y-1 pb-1">
                                <CardTitle className="text-sm text-foreground">핵심 지표 레이더</CardTitle>
                                <p className="text-xs text-muted-foreground">최근 4개 분기 지표를 정규화한 점수</p>
                            </CardHeader>
                            <Separator className="bg-border/40" />
                            <CardContent className="flex flex-col items-center gap-4 p-5">
                                <RadarChart
                                    metrics={company.metrics}
                                    axes={companyMetricKeys.map((key) => ({
                                        key,
                                ...
                                <p className="text-center text-xs leading-relaxed text-muted-foreground/80">
                                    Donut Grade의 다섯 축을 동일한 축척으로 비교해 특정 영역의 강·약점을 빠르게 파악할 수 있습니다.
                                </p>
                            </CardContent>
                        </Card>
                        ...
                        <Card className="border border-border/30 bg-card/70 backdrop-blur">
                            <CardHeader className="pb-2">
                                <CardTitle className="text-sm text-foreground">Donut Grade 구성 지표</CardTitle>
                            </CardHeader>
                            <CardContent className="grid gap-3 p-5 sm:grid-cols-2 xl:grid-cols-3">
                                {companyMetricKeys.map((key) => (
                                    <div
                                        key={key}
                                        className="rounded-2xl border border-border/40 bg-muted px-4 py-3 text-sm text-muted-foreground"
                                    >
                        ...
                        <Card className="border border-border/30 bg-card/70 backdrop-blur">
                            <CardHeader className="pb-2">
                                <CardTitle className="text-sm text-foreground">성장 하이라이트</CardTitle>
                            </CardHeader>
                    ...
                    <div className="space-y-6">
                        <Card className="border border-border/30 bg-card/70 backdrop-blur">
                            <CardHeader className="pb-2">
                                <CardTitle className="text-sm text-foreground">기업 정보</CardTitle>
                            </CardHeader>
                            <CardContent className="space-y-4 p-5 text-sm text-muted-foreground">
                                ...
                                <Separator className="bg-border/40" />
                                <div className="flex flex-wrap gap-2 text-[0.65rem] uppercase tracking-[0.2em] text-muted-foreground/70">
                                    {company.focus.map((tag) => (
                                        <span
                                            key={tag}
                                            className="rounded-full border border-border/30 bg-muted px-3 py-1 font-semibold text-muted-foreground"
                                        >
                                            #{tag}
                                        </span>
                                    ))}
                        ...
                        <Card className="border border-border/30 bg-card/70 backdrop-blur">
                            <CardHeader className="pb-2">
                                <CardTitle className="text-sm text-foreground">빠른 메모</CardTitle>
                            </CardHeader>
                            <CardContent className="space-y-4 p-5 text-sm leading-relaxed text-muted-foreground">
                                <p>
                                    Donut Grade는 성장력·수익성·재무 안정성·모멘텀·밸류/리스크를 0~100점으로
                                    정규화해 산출합니다. 업계 평균과 비교해 70점 이상이면 상위 20% 수준으로
                                    평가됩니다.
                                </p>
                                <Button variant="ghost" className="px-0 text-primary" onClick={handleBack}>
                                    목록으로 돌아가기
                                </Button>
                            </CardContent>
                        </Card>
