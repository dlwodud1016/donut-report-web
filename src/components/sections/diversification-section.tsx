...
export function DiversificationSection(): JSX.Element {
  ...
  return (
    <Card id="section-2-2" className="bg-card p-8 shadow-lg dark:bg-slate-900/70 dark:shadow-2xl">
      ...
      <CardContent className="space-y-8 p-0">
        <section className="grid gap-6 lg:grid-cols-2">
          <article className="space-y-4 rounded-3xl border border-sky-400/30 bg-card p-6 shadow-lg dark:border-sky-400/40 dark:bg-slate-900/60 dark:shadow-xl dark:shadow-black/30">
            <header>
              <h3 className="text-lg font-semibold text-foreground">산업별 매출 비중</h3>
              <p className="mt-1 text-sm text-sky-700 dark:text-sky-100/80">방위·민간 수요가 함께 성장하면서 수익 안정성과 업셀 파이프라인 모두를 확보했습니다.</p>
            </header>
            <div className="relative flex flex-col items-center gap-5 rounded-2xl border border-border/30 bg-card px-6 py-6 dark:border-white/10 dark:bg-slate-900/70">
              <div
                ...
                aria-hidden="true"
              >
                <div className="absolute inset-[22%] flex flex-col items-center justify-center rounded-full border border-white/30 bg-white/90 text-center text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-700 dark:border-white/10 dark:bg-slate-950/80 dark:text-slate-100/80">
                  <span>Revenue</span>
                  <span className="mt-1 text-xs tracking-normal text-slate-600 dark:text-slate-100/75">Mix</span>
                </div>
              </div>
              <ul className="grid w-full grid-cols-1 gap-3 text-[11px] uppercase tracking-wide text-slate-700 dark:text-slate-100/80 sm:grid-cols-3">
                {verticalMix.map((segment) => (
                  <li key={segment.label} className="flex flex-col items-center gap-1 rounded-xl border border-border/30 bg-white/80 p-2 text-center shadow-inner dark:border-white/10 dark:bg-white/5">
                    <span
                      className="h-2 w-2 rounded-full"
                      style={{ backgroundColor: segment.color }}
                      aria-hidden="true"
                    />
                    <span className="text-[10px] text-slate-600 dark:text-slate-100/70">{segment.label}</span>
                    <span className="text-sm font-semibold text-slate-800 dark:text-slate-100">{Math.round((segment.value / mixTotal) * 100)}%</span>
                  </li>
                ))}
              </ul>
            </div>
            <ul className="space-y-2 rounded-2xl border border-border/30 bg-secondary/70 p-4 text-sm text-slate-700 dark:border-white/10 dark:bg-slate-950/40 dark:text-slate-200/80">
              <li>• 정부·국방 프로젝트로 전체 매출의 절반을 안정적으로 확보</li>
              <li>• 제조·에너지 고객에게 “예지 정비” 모듈을 패키지로 판매 중</li>
            </ul>
          </article>
          <article className="space-y-4 rounded-3xl border border-primary/30 bg-card p-6 shadow-lg dark:border-primary/40 dark:bg-slate-900/60 dark:shadow-xl dark:shadow-black/30">
            <header>
              <h3 className="text-lg font-semibold text-foreground">고객당 평균 매출 증감 (YoY)</h3>
              <p className="mt-1 text-sm text-muted-foreground dark:text-purple-100/80">기존 고객이 AIP 모듈을 덧붙이며 연평균 18%씩 지출이 늘고 있습니다.</p>
            </header>
            <div className="relative min-h-[13rem] rounded-2xl border border-border/30 bg-card px-4 py-6 dark:border-white/10 dark:bg-slate-900/70">
              <div className="absolute left-5 top-5 flex flex-col gap-1 rounded-xl border border-border/30 bg-white/85 px-3 py-2 text-xs text-slate-700 dark:border-white/10 dark:bg-white/10 dark:text-slate-100">
                <span className="uppercase tracking-[0.18em] text-slate-500 dark:text-slate-300">NRR Trend</span>
                ...
                {arpuPoints.map((point) => (
                  <g key={point.label}>
                    ...
                    <text
                      ...
                      y={point.y - 12}
                      textAnchor="middle"
                      className="fill-foreground text-[11px] font-semibold dark:fill-slate-100"
                    >
                      {point.percent}
                    </text>
                    <text
                      ...
                      textAnchor="middle"
                      className="fill-muted-foreground text-[10px] uppercase tracking-[0.18em] dark:fill-slate-300"
                    >
                      {point.label}
                    </text>
                  </g>
            ...
            <ul className="space-y-2 rounded-2xl border border-border/30 bg-secondary/70 p-4 text-sm text-muted-foreground dark:border-white/10 dark:bg-slate-950/40 dark:text-slate-200/80">
              <li>• FY25 기존 고객 순증가율(NRR) 113% → 업셀 효과 지속</li>
              <li>• AIP 워크플로 구축 건수 YoY +32%로 ARPU 확대를 견인</li>
            </ul>
          </article>
        </section>
        <ul className="space-y-3 rounded-3xl border border-border/40 bg-secondary/70 p-6 text-sm text-muted-foreground dark:bg-slate-950/60">
